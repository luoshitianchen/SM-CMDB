# 配置管理数据库（CMDB）服务 上线部署检查清单（DEPLOYMENT_CHECKLIST）
...
- [ ] ConfigMap 环境变量正确（SM_ENV / SM_DATABASE_NAME=sm_cmdb）
...
1. `helm lint helm/sm-cmdb`
2. `helm upgrade --install sm-cmdb helm/sm-cmdb -n sm-prod -f helm/sm-cmdb/values-prod.yaml`
3. 等待 rollout：`kubectl -n sm-prod rollout status deploy/sm-cmdb`
